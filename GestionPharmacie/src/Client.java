...
public class Client extends Pharmacien {
	...
	//Modifier Client
	public void modifierClient(ArrayList<Client> ListClient, int id, String Nom,String prenom,String email,String telephone,int nombrAchat,String badge) {
		for(int j=0;j<ListClient.size();j++) {
			if(id==ListClient.get(j).getId()) {
				...
				System.out.println("Modifier avec succes id =" + id + " and Clients : \n" + ListClient.get(j).toString());
			}else {
	...
	static int checkClientIfExist(ArrayList<Client> ListClient, int id) {
		int a;
		a = 0;
		for(int j=0;j<ListClient.size();j++) {
			if(id==ListClient.get(j).getId()) {
	...
	static int checkClientIfExist2(ArrayList<Client> ListClient, String Nom) {
		int a;
		int bdg = 0;
		int bdg2 = 0;
		a = 0;
